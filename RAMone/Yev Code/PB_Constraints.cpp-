...
void rcfcn_beginning(double *ts, double *sd, double *sa, double *u, double *p_free,
	...
	res[lL_const]       = sd[lL];
	res[dlL_const]      = sd[dlL];
	res[ulL_const]      = sd[ulL];
	res[dulL_const]     = sd[dulL];
	res[lR_const]       = sd[lR];
	res[dlR_const]      = sd[dlR];
	res[ulR_const]      = sd[ulR];
	res[dulR_const]     = sd[dulR];
...
void rcfcn_endSymmetric(double *ts, double *sd, double *sa, double *u,
	...
	res[lL_const]       = -sd[lR];
	res[dlL_const]      = -sd[dlR];
	res[ulL_const]      = -sd[ulR];
	res[dulL_const]     = -sd[dulR];
	res[lR_const]       = -sd[lL];
	res[dlR_const]      = -sd[dlL];
	res[ulR_const]      = -sd[ulL];
	res[dulR_const]     = -sd[dulL];
...
void figureview_output(long *imos, ///< index of model stage (I)
	...
	fprintf(fp, "0\t");
	fprintf(fp, "0\t");
	fprintf(fp, "%e\t", 1 - sd[lR]);
	fprintf(fp, "0\t");
	fprintf(fp, "%e\t", 1 - sd[lL]);
	fprintf(fp, "\n");
	// Close file
	fclose(fp);
}
